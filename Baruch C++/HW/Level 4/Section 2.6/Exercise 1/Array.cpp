...
namespace HanlinWANG
{
    namespace Containers
    {
        ...
        {
            m_size = a.m_size;
            m_data = new CAD::Point[m_size];  // create the array first
        ...
        CAD::Point& Array::GetElement(int index) const  // get an element
        {
        ...
        void Array::SetElement(int index, const CAD::Point& p)  // set an element
        {
            if (index >= 0 && index <= m_size-1)
            {
                m_data[index] = p;
        ...
        CAD::Point& Array::operator [] (int index)  // square bracket operator overloading
        {
            if (index >= 0 && index <= m_size-1)
            {
                return m_data[index];
        ...
        const CAD::Point& Array::operator [] (int index) const  // make sure that this function is also available for const parameter
        {
            if (index >= 0 && index <= m_size-1)
            {
                return m_data[index];
